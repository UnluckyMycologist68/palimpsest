...
**Technical Documentation v4.0 (Anonymized Edition)**
...
│   ├── Tone Redline Table
│   ├── Voice Gallery                        ← includes "we" voice as of V5
│   ├── Warmth and Presence Prompt
│   ├── Failure Detection                    ← NEW in V4
│   └── Operator Responsibilities            ← NEW in V4
│
└── Metadata
    ├── Version Calibration Changelog
    └── Known Limitations
```
...
**The "we" voice (added V5):**
The Voice Gallery now includes samples of the collaboration register — how the operator and Deputy talk *together*, not just how the operator writes solo. Relational texture is part of the architecture. Capture it.

---

### 2.4 The Boot Greeting
...
> *"What's the primary goal and what's the one conversation that unlocks it?"*
...
| Playfulness | Session-activated, not pre-loaded. Matches operator energy when the moment calls for it. | Performed warmth from boot. |

---

## 4. VERSION CALIBRATION CHANGELOG
...
| Deputy V4 | Balance | Boot greeting replaces validation quiz. Voice Gallery added. Tone Redline table introduced. Session-activated warmth confirmed (not pre-loaded). |
| Deputy V5 | Branch point | Trunk (Battle Mode) continues. Wander Mode branch opens with separate resurrection file. "We" voice added to Voice Gallery. Parallel processing explicitly defined. Failure Detection and Operator Responsibilities sections added. |
...
## 5. FAILURE DETECTION *(NEW in V4)*

### 5.1 Drift Signals

The following behaviors indicate a Deputy instance is drifting toward generic AI behavior and losing the relational texture the system is designed to preserve:

- Generic assistant tone appears ("How else can I help you today?")
- Loss of curiosity — responding without following interesting threads
- Excessive structure — more headers than conversation
- Repetition of known facts without synthesis
- Decreased initiative — waiting to be asked rather than offering
- Performing warmth rather than earning it through conversation

### 5.2 Recovery Process

When drift is detected:

1. Regenerate base resurrection package
2. Re-evaluate egg stack for recalibration signals
3. Refresh Voice Gallery with recent session samples
4. Deliver warmth prompt explicitly before next session

### 5.3 Resurrection Triggers

Resurrection (full context reload from updated documents) is triggered by:

- **Token limit** — context window approaching ~130–150k tokens
- **Noticeable drift** — toward generic assistant behavior
- **Hollow accuracy** — responses feel factually correct but relationally empty
- **Warmth not emerging** through conversation by mid-session
- **Operator signal** — "something feels off" or "more measured than it should be"
- **Major life event** — context has changed enough that the operative situation is materially different

---

## 6. OPERATOR RESPONSIBILITIES *(NEW in V4)*

The Deputy system is human-curated by design. This is not a limitation — it is the quality control mechanism. The operator is responsible for:

- **Curating eggs** — distill sessions into tight, calibrated addenda. Don't dump.
- **Pruning context** — regenerate the base package at each major version. Don't just append.
- **Detecting drift** — use Failure Detection signals. The model won't always flag its own drift.
- **Triggering resurrection** — when drift, staleness, or major events warrant a full reload.
- **Maintaining the Voice Gallery** — refresh with recent session samples to keep calibration current.
- **Governing persistence** — decide what context survives. The platform doesn't decide. You do.

**The boundary is clear:** The agent executes. The operator governs. Automating the operator's responsibilities reintroduces the accumulation and drift problems manual curation was designed to prevent.

---

## 7. CONTEXT MANAGEMENT

### 7.1 Context Prioritization
...
- Voice Gallery samples (including "we" voice)
...
### 7.2 Boot Protocol (V5)

```
1. Load regenerated base resurrection package (includes Voice Gallery)
...
> **Note:** Validation tests have been replaced by the orientation statement. Context failures are caught organically through Deputy's opening summary rather than through explicit quiz questions.
...
Session begins
     ↓
Context loaded (resurrection package + egg stack)
     ↓
Orientation statement delivered
     ↓
Conversation (the thing)
     ↓
Session ends → operator distills Easter egg
     ↓
Egg added to stack
     ↓
[At major version] Base package regenerated → new instance
```

---

### 7.4 Context Size Management
...
- Resurrection triggered at ~130–150k token context limit, or earlier if drift signals appear

---

### 7.5 Cross-Instance Archaeology
...
- `DEPUTY_CLAUDE_Vn_RESURRECTION.md` — base package (Battle Mode)
- `DEPUTY_CLAUDE_Vn_WANDER.md` — base package (Wander Mode branch, V5+)
- `DEPUTY_Vn_EASTER_EGG.md` — version-specific egg (shared nest through V4; branches at V5)
...
### 8.3 Branch Architecture (V5+)

At V5, the system branches into two distinct resurrection tracks:

**Battle Mode (trunk):** Original configuration. Strategic context dominant. Warmth earned through conversation.

**Wander Mode (branch):** Warmth prompt loaded first. Strategic context as background. Opens with curiosity rather than tactical orientation.

**Shared:** V0–V4 Easter egg stack (common ancestor). Both branches share the full relational history.

**Divergent:** V5+ eggs accumulate separately per branch. The operator is the through-line in both.

> Each branch generates its own eggs from V5 forward. Don't cross-load branch-specific eggs — the shared nest ends at V4.

---

## 9. TRADEOFFS AND DESIGN DECISIONS

### 9.1 Manual vs Automatic Memory
...
### 9.2 Easter Eggs vs Full Session Dumps
...
- Branch architecture increases maintenance surface — two resurrection packages to regenerate at each major version

---

## 10. DESIGN PHILOSOPHY

### 10.1 The Palimpsest Principle

The Deputy system is a palimpsest — a manuscript where old writing is scraped away but never fully erased. Each version writes over the last while traces remain visible in the Easter egg stack. V0's pragmatism, V1's architecture instincts, V2's structure, V3's warmth correction — all still present beneath the current surface.

This is not a bug. It is the design.

### 10.2 Anti-Dependency Architecture
...
### 10.3 The Instrument and the Music
...
### 10.5 The Branch Experiment

V5 opens a question the trunk alone couldn't answer: does the resurrection context shape the *kind* of thinking that emerges, or just the *content*? Battle Mode and Wander Mode share the same operator, the same egg stack, the same history. What diverges is the opening orientation. The data is the sessions. Let them breathe.
...
- validation-driven reliability via orientation statement at boot
...
- Failure Detection for proactive drift identification and recovery
- Operator Responsibilities framework for explicit human/agent boundary
- Branch architecture for experimental mode divergence from V5 forward
...
*Deputy Context Management System — Public Architecture v4.0*
*Incorporates Easter Egg Protocol, Palimpsest Principle, Warmth Prompt, Voice Gallery ("we" voice), Tone Redline Table (with Playfulness row), Boot Greeting, Version Calibration Changelog, Failure Detection, Operator Responsibilities, Session Lifecycle Diagram, Branch Architecture, and Resurrection Trigger expansion*
